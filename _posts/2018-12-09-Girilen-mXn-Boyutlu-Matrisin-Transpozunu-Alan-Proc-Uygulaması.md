...
inputları Inputs dizisine atar ve son input hariç inputların karakter sayılarını CharacterNumbersOfInputs
dizisine atar.
	    {
	        if (ProcBuffer[i]==',')
		    {
	...
	CharacterNumbersOfInputs[l]=Size-1-k; // Son inputun karakter sayısını CharacterNumbersOfInputs
dizisine atar.
	Index=0;
	Size=Size*2+24; // ProcBuffer dizisine matris ve transpoz matris atılacak olduğundan dolayı Size ı 2
ile çarpar ve ProcBuffer dizisine "Matrix" ve "Transpose Matrix" başlıkları atılacak olduğundan dolayı 24 ile
toplar.
	...
	for (i=0;i<=(m*n)-1;i++)
	    {
	        for (j=0;j<=CharacterNumbersOfInputs[i]-1;j++) // Matrisin elemanlarını yani inputları ProcBuffer
dizisine atar.
		    {
		        Index++;
